    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
       ...
       cell.ProductPrice.text =  ((((self.Products[indexPath.row + 1]) as AnyObject)["Price"]) as! String)
        for i in (cart?.cartProducts)!{
            print("this:\((((self.Products[indexPath.row + 1]) as AnyObject)["id"]) as! String)")
            print(i)
            if((i) == ((((self.Products[indexPath.row + 1]) as AnyObject)["id"]) as! String)){
                print("\(i): here")
                cell.Cart.image = #imageLiteral(resourceName: "InCart")
                break
            }
            else{
                 cell.Cart.image = #imageLiteral(resourceName: "AddCart")
            }
        }
        for i in (list?.listProducts)!{
            print("this:\((((self.Products[indexPath.row + 1]) as AnyObject)["id"]) as! String)")
            print(i)
            if((i) == ((((self.Products[indexPath.row + 1]) as AnyObject)["id"]) as! String)){
                print("\(i): here")
                cell.Like.image = #imageLiteral(resourceName: "Liked")
                break
            }
            else{
                cell.Like.image = #imageLiteral(resourceName: "Unliked")
            }
        }
